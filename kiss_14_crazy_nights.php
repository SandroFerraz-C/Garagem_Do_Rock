<!DOCTYPE html>
<!-- doctype informa ao agente de usuario a versao do tml que dee ser rederizada -->

<html lang="pt-br">

<head>

    <title>Garage Of Rock!!</title>

    <meta charset="utf-8">
    <meta name="Author" content="Sandro">
    <meta name="descrition" content="Lista de documentos">
    <meta name="keywords" content="html5, tecnologia">

    <!-- Latest compiled and minified CSS -->

    <link rel="stylesheet" type="text/css" href="http://yui.yahooapis.com/3.3.0/build/cssreset/reset-min.css">
    <link rel="stylesheet" type="text/css" href="css/hv_2.css">
    <link rel="stylesheet" type="text/css" href="css/layer_kiss.css">


    <!--
    <link rel="stylesheet" type="text/css" href="css/ti.css">
    <link rel="stylesheet" type="text/css" href="css/tm.css">
    <link rel="stylesheet" type="text/css" href="css/lc.css">
    <link rel="stylesheet" type="text/css" href="css/cp.css">
    <link rel="stylesheet" type="text/css" href="css/gn.css">
    <link rel="stylesheet" type="text/css" href="css/bm.css">
-->
</head>

<body>
    <br>

    <main>
        <section class="content-section">

            <h1>Crazy Nights (1987)</h1>
            <br />

            <p>
                - Crazy Nights é o decimo quarto álbum do Kiss, gravado de março a junho de 1987.
            </p>
            <br />

            <p>
                - Crazy Nights também é o primeiro álbum do KISS a apresentar o uso pesado de teclados e sintetizadores, o que foi outra
                mudança em seu estilo musical, mudando de seu som de heavy metal de - Creatures of the Night / - Lick It Up / - Animalize /
                - Asylum para um mais amigável ao rádio e som de metal pop. Foi relançado em 1998 como parte da série Kiss Remasters e é o
                último álbum do Kiss a ser remasterizado.
            </p>
            <br />

            <p>
                Um número relativamente alto de músicas do - Crazy Nights foi tocado ao vivo durante a turnê de divulgação, e
                especialmente imediatamente após a turnê, a maioria dessas músicas foi abandonada e nunca foi tocada novamente. Apenas a música
                "Crazy Crazy Nights" foi mantida em seu setlist para a - Hot in the Shade Tour, que se seguiu alguns anos depois. Isso torna o
                álbum um dos menos representados em todo o catálogo da banda ao longo de sua carreira em suas setlists, atrás apenas do fracasso
                comercial de 1981, - Music from "The Elder" .
            </p>
            <br />

            <p>
                Kiss adotou uma abordagem diferente na criação de seu álbum - Crazy Nights, em um esforço para mudar sua imagem depois que eles
                tiveram uma queda em sua carreira musical devido à experimentação do gênero musical da banda e à perda de dois membros principais.
                A banda tinha muito a provar após seu declínio de sucesso.
            </p>
            <br /><br />

            <p>Duração do album: 42 min. 58 seg.</p>
            <p>Gravadora(s): Mercury Records (EUA) e Vertigo Records (Europa).</p>
            <p>Produção: Ron Nevison.</p>
            <br /><br />

            <p>
                O album atingil certificação:
                <br /><br />

            <div class="tabela">
                <table border="1px" width="45%">

                    <tr>
                        <td width="15%" height="10%">Nivel</td>
                        <td width="15%">Albuns Vendidos</td>
                    </tr>


                    <tr>
                        <td width="15%"> Platina </td>
                        <td> + 1.000.000 (EUA.)</td>
                    </tr>
                    <tr>
                        <td width="15%"> Platina </td>
                        <td> + 100.000 (Canadá)</td>
                    </tr>
                    <tr>
                        <td width="15%"> Ouro </td>
                        <td> + 50.000 (Finlândia)</td>
                    </tr>


                </table>
            </div>
            </p>
            <br /><br />


            <section class="disco">
                <table id="demo-table">

                    <iframe src="https://open.spotify.com/embed/playlist/4ZbL1Yyn3mVtyVkWoqA0ey" width="100%" height="650" frameborder="0" allowtransparency="true" allow="encrypted-media"></iframe>
                    <br /><br />

                    <ul>
                        <li><a href="#" onMouseOver="mostraLayer('divPS14');" onMouseOut="escondeLayer('divPS14');"> Paul Stanley</a> – Vocal e Guitarra Base. </li>
                        <li><a href="#" onMouseOver="mostraLayer('divBK14');" onMouseOut="escondeLayer('divBK14');"> Bruce Kulick </a> – Guitarra Principal. </li>
                        <li><a href="#" onMouseOver="mostraLayer('divGS14');" onMouseOut="escondeLayer('divGS14');"> Gene Simmons</a> – Vocal e Baixo. </li>
                        <li><a href="#" onMouseOver="mostraLayer('divEC14');" onMouseOut="escondeLayer('divEC14');"> Eric Carr </a> – Bateria. </li>
                    </ul>

                    <div id="divPS14" name="nm_divPS14">
                        <p>
                            <img class="css" src="img/kiss/ps.jpg" width="150px;" height="130px;">
                            Paul Stanley é um cantor, compositor e pintor americano, mais conhecido por ser o co-fundador, vocalista,
                            guitarrista rítmico e co-vocalista ddo Kiss. Ele é o escritor ou co-escritor de muitos dos maiores sucessos da banda.
                            Stanley estabeleceu o personagem Starchild e é conhecido por sua voz distinta e abrangente.
                        </p>
                    </div>


                    <div id="divBK14" name="nm_divBK14">
                        <p>
                            <img class="css" src="img/kiss/bk.jpg" width="150px;" height="130px;">
                            Bruce Howard Kulick é um guitarrista, músico americano e desde 2000 membro da banda Grand Funk Railroad.
                            Anteriormente, Kulick foi um membro de longa data da banda Kiss (1984–1996). Ele também foi membro do Union com
                            John Corabi de 1997–2002 e Blackjack de 1979–1980.
                        </p>
                    </div>

                    <div id="divGS14" name="nm_divGS14">
                        <p>
                            <img class="css" src="img/kiss/gs.jpg" width="150px;" height="130px;">
                            Gene Klein, conhecido profissionalmente como Gene Simmons, é um músico, cantor, compositor, produtor musical,
                            empresário, ator, autor e personalidade da televisão americana. Também conhecido por seu personagem de palco,
                            The Demon, ele é o baixista e co-vocalista do Kiss, a banda de rock que ele co-fundou com Paul Stanley no início dos
                            anos 1970.
                        </p>
                    </div>

                    <div id="divEC14" name="nm_divEC14">
                        <p>
                            <img class="css" src="img/kiss/ecarr.jpg" width="150px;" height="130px;">
                            Paul Charles Caravello, "Eric Carr", ara um musico multi-instrumentista que foi baterista da banda de rock Kiss de 1980 a 1991. Caravello foi escolhido como o novo baterista
                            do Kiss após a saída de Peter Criss, quando escolheu o nome artístico de "Eric Carr". Ele permaneceu como membro
                            do Kiss até sua morte de câncer no coração em 24 de novembro de 1991, aos 41 anos.
                        </p>
                    </div>
                    <br /><br />

                    <a href="kiss.php"><button>Voltar para Albuns do Kiss</button></a>
                    <a href="index.php"><button>Home</button></a>

                    </div>
            </section>

    </main>
    </div>
    </div>

    <script type="text/javascript" src="js/Country.js"></script>


</body>

</html>