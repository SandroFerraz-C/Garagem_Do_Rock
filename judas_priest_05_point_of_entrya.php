<!DOCTYPE html>
<!-- doctype informa ao agente de usuario a versao do tml que dee ser rederizada -->

<html lang="pt-br">

<head>

    <title>Garage Of Rock!!</title>

    <meta charset="utf-8">
    <meta name="Author" content="Sandro">
    <meta name="descrition" content="Lista de documentos">
    <meta name="keywords" content="html5, tecnologia">

    <!-- Latest compiled and minified CSS -->

    <link rel="stylesheet" type="text/css" href="http://yui.yahooapis.com/3.3.0/build/cssreset/reset-min.css">
    <link rel="stylesheet" type="text/css" href="css/hv_2.css">
    <link rel="stylesheet" type="text/css" href="css/layer_judas_priest.css">


    <!--
    <link rel="stylesheet" type="text/css" href="css/ti.css">
    <link rel="stylesheet" type="text/css" href="css/tm.css">
    <link rel="stylesheet" type="text/css" href="css/lc.css">
    <link rel="stylesheet" type="text/css" href="css/cp.css">
    <link rel="stylesheet" type="text/css" href="css/gn.css">
    <link rel="stylesheet" type="text/css" href="css/bm.css">
-->
</head>

<body>
    <br>

    <main>
        <section class="content-section">

            <h1>Point of Entry (1981)</h1>
            <br />

            <p>
                - Point of Entry é o sétimo álbum de estúdio da banda Judas Priest, lançado em 26 de Fevereiro de 1981.
            </p>
            <br />

            <p>
                A banda seguiu uma direção mais amigável para ser tocado em ao rádio em - Point of Entry . Após a conclusão da British
                Steel Tour , a banda começou a trabalhar em seu próximo projeto. A essa altura, eles tinham fundos suficientes para levar
                todo o seu equipamento aos estúdios de última geração Ibiza Studios, na Espanha. Isso deu ao Point of Entry um som mais alto,
                forte e mais "ao vivo" do que os álbuns anteriores do Judas Priest.
            </p>
            <br />

            <p>
                A capa do álbum foi projetada por Roslav Szaybo, que fez todos os álbuns da banda na CBS até então. "A capa norte-americana
                diferiu do resto do mundo. A arte americana, com papel de impressora de computador para
                simular a linha no meio da estrada e caixas de papelão brancas na parte de trás, foi desenhada por John Berg, da Columbia Record.
                "'A capa estava horrível, zomba do guitarrista (Glenn Tipton)', e temos que culpar a gerência por isso porque eles não pesquisaram o
                suficiente para encontrar uma que fosse adequada. A capa americana era diferente, mas isso acabou sendo ainda pior! '"A arte americana
                também viu a introdução do logotipo 3D do Judas Priest, que seria usado até o álbum - Turbo .
            </p>
            <br /><br />

            <p>Duração do album: 37 min. 42 seg.</p>
            <p>Gravadora(s): Columbia Records.</p>
            <p>Produção: Judas Priest e Tom Allom.</p>
            <br /><br />

            <p>
                O album atingil certificação:
                <br /><br />

            <div class="tabela">
                <table border="1px" width="45%">

                    <tr>
                        <td width="15%" height="10%">Nivel</td>
                        <td width="15%">Albuns Vendidos</td>
                    </tr>



                    <tr>
                        <td width="15%"> Ouro </td>
                        <td> + 500.000 (EUA.)</td>
                    </tr>
                    <tr>
                        <td width="15%"> Prata </td>
                        <td> + 60.000 (Reino Unido)</td>
                    </tr>

                </table>
            </div>
            </p>
            <br /><br />


            <section class="disco">
                <table id="demo-table">

                    <iframe src="https://open.spotify.com/embed/playlist/3NIQknTXD0c3lPdgtO2Apa" width="100%" height="690" frameborder="0" allowtransparency="true" allow="encrypted-media"></iframe>
                    <br /><br /><br />

                    <ul>
                        <li><a href="#" onMouseOver="mostraLayer('divRH05');" onMouseOut="escondeLayer('divRH05');"> Rob Halford</a> – Vocal. </li>
                        <li><a href="#" onMouseOver="mostraLayer('divkkd05');" onMouseOut="escondeLayer('divkkd05');"> K. K. Downing</a> – Guitarra. </li>
                        <li><a href="#" onMouseOver="mostraLayer('divGT05');" onMouseOut="escondeLayer('divGT05');"> Glenn Tipton</a> – Guitarra e Piano. </li>
                        <li><a href="#" onMouseOver="mostraLayer('divIH05');" onMouseOut="escondeLayer('divIH05');"> Ian Hill</a> – Baixo. </li>
                        <li><a href="#" onMouseOver="mostraLayer('divDH05');" onMouseOut="escondeLayer('divDH05');"> Dave Holland</a> – Bateria. </li>
                    </ul>

                    <div id="divRH05" name="nm_divRH05">
                        <p>
                            <img class="css" src="img/judas priest/rh.jpg" width="150px;" height="130px;">
                            Robert John Arthur Halford é um cantor e compositor inglês. Ele é mais conhecido como o vocalista da banda de
                            heavy metal Judas Priest, que foi formada em 1969 e recebeu elogios como o Prêmio Grammy de 2010 de Melhor
                            Performance de Metal. Ele tem se destacado por sua voz poderosa e abrangente e sua imagem de marca registrada em
                            couro e tachas, que se tornaram icônicas no heavy metal.
                        </p>
                    </div>

                    <div id="divkkd05" name="nm_divkkd05">
                        <p>
                            <img class="css" src="img/judas priest/kkd.jpg" width="150px;" height="130px;">
                            Kenneth "K. K." Downing Jr. é um músico inglês. Ele é co-fundador e ex-guitarrista da banda de heavy metal Judas Priest.
                            <br />
                            Downing deixou o Judas Priest oficialmente em 20 de abril de 2011, citando "um colapso contínuo nas relações de
                            trabalho entre mim, elementos da banda e empresários". Ele afirmou que havia pelo menos "21 razões" pelas quais ele
                            saiu, mas se recusou a entrar em detalhes específicos;
                        </p>
                    </div>

                    <div id="divGT05" name="nm_divGT05">
                        <p>
                            <img class="css" src="img/judas priest/gt.jpg" width="150px;" height="130px;">
                            Glenn Tipton é um músico inglês. Muito conhecido por seu estilo de tocar complexo e solos influenciadores, ele é
                            mais conhecido como um dos guitarristas do Judas Priest.
                            <br />
                            Tipton aprendeu a tocar violão aos 19 anos, com seu primeiro violão. Ele então o trocou em uma guitarra simples
                            até poder comprar uma Fender Stratocaster. A fender se tornou sua guitarra principal em shows ao vivo, até que foi roubada
                            em um show.
                        </p>
                    </div>

                    <div id="divIH05" name="nm_divIH05">
                        <p>
                            <img class="css" src="img/judas priest/ih.jpg" width="150px;" height="130px;">
                            Ian Frank Hill é um músico inglês, mais conhecido como baixista da banda de heavy metal Judas Priest.
                            <br />
                            Hill aprendeu a tocar contrabaixo com seu pai, um baixista de shows de jazz locais. Em 1970, junto com seu colega
                            de escola K. K. Downing, Hill formou a banda de heavy metal Judas Priest. Ele toca baixo com a banda desde então
                            e é o único membro original continuamente na banda desde o início, após a saída de Downing em 2011.
                        </p>
                    </div>

                    <div id="divDH05" name="nm_divDH05">
                        <p>
                            <img class="css" src="img/judas priest/dh.jpg" width="150px;" height="130px;">
                            David Holland foi um baterista de heavy metal inglês.
                            <br />
                            Aos seis anos, Holland começou a estudar piano, mas logo desenvolveu uma "mania pela bateria" em suas próprias
                            palavras e implorou a seus pais que o deixassem fazer um show. Após sua primeira aparição como substituto de uma
                            banda local, Holland percebeu que queria ser músico. Quando tinha 14 anos, ele complementou sua mesada tocando
                            com outra banda local chamada The Drumbeats.
                        </p>
                    </div>
                    <br /><br />

                    <a href="judas_priest.php"><button>Voltar para Albuns do Judas Priest</button></a>
                    <a href="index.php"><button>Home</button></a>

                    </div>
            </section>

    </main>
    </div>
    </div>

    <script type="text/javascript" src="js/Country.js"></script>


</body>

</html>